...
**2026-08-31 — pipeline milestone 1 built (credential-free), ontology+grounding adopted.**
Components `warmhub-data/ontology` 2.0.0 + `warmhub-data/grounding` 4.0.0 installed on the catalog (doctor ok); 34 method records seeded via `catalog/ontology/build_ontology_ops.py` — charter, 8 CompetencyQuestions, 14 contracts (Set/ontology/release@v1), 4 GoldenCases with live exemplars, 4 OntologyDecisions (od-discontinuation-semantics and od-provenance-depth resolved), 2 grounding Sources. Pipeline repo: **github.com/WJCorey/carrera-catalog-pipeline** (mushroom template @ ccb29a2, private) — carrera-toys.com ProductListing grounding stream, scope `ProductListing/grounding/carrera/toys-shop/**`, preserve-only; 107 tests; live `prepare_fresh_sync` green (1,463 products; the first run failed closed on SKU `29920402B` and the identity alphabet was widened from evidence). ~~Gate: SPEC human-approval checkpoint (Corey) before shape provisioning, token, validate_sync, synchronize.~~ Source win: carrera-toys.com robots.txt points at an `/agents.md` that explicitly sanctions read-only product JSON.

**2026-09-01 — SPEC approved (Corey); pipeline m1 + m2 LIVE and automated. Phase 4 pipeline: DONE.**
`ProductListing@v2` + contracts provisioned (`Set/ontology/release@v3`, loader `catalog/ontology/build_product_listing_v2_ops.py`); scoped PAT minted (write limited to `ProductListing/grounding/**` + the two SourceArtifact streams). First `synchronize` converged 1,463 carrera-toys listings (empty residual); m2 added the carreraslots.com sitemap-presence stream (734 parts, one request — the requested 20 s crawl-delay makes page crawling a deliberate future milestone) with 1,463 unchanged on resync proving the no-op property. `SourceArtifact` stream Things live via the idempotent sidecar (`scripts/record_artifact.py`), canonical artifacts committed content-addressed in the pipeline repo. Deployment: GitHub Actions monthly cron + manual button (serialized, tests gate the write, `.mushroom/` retained 30 d). Two fail-closed identity discoveries recorded in the SPEC (toys SKU `29920402B`; slots part `GC_50`). Remaining Phase 4: blog post + demo. Handover: pipeline repo `WJCorey/carrera-catalog-pipeline` is self-contained (SPEC, RUNLOG, workflow, token-rotation note).
